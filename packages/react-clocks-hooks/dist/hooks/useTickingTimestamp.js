...
*/
const useTickingTimestamp = function useTickingTimestamp() {
  const _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const _ref$timestamp = _ref.timestamp;
  const timestamp = _ref$timestamp === void 0 ? Date.now() : _ref$timestamp;
  const _ref$interval = _ref.interval;
  const interval = _ref$interval === void 0 ? 1000 : _ref$interval;

  const _useState = (0, _react.useState)(timestamp);
  const _useState2 = _slicedToArray(_useState, 2);
  const tickingTimestamp = _useState2[0];
  const setTickingTimestamp = _useState2[1];
...
const _default = useTickingTimestamp;
